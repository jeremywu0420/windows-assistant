...
function getCpuClockGhz() {
  const cpus = os.cpus();
  const speeds = cpus.map((cpu) => toNumber(cpu.speed)).filter((value) => value != null && value > 0);
  ...
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      (err, stdout) => {
        ...
        const [usage, temperature, memoryUsedMb, memoryTotalMb] = line.split(',').map((part) => part.trim());
        resolve({
          usagePercent: round(usage, 0),
          temperatureC: round(temperature, 0),
          vramUsedBytes: toNumber(memoryUsedMb) == null ? null : Math.round(toNumber(memoryUsedMb) * 1024 * 1024),
          vramTotalBytes: toNumber(memoryTotalMb) == null ? null : Math.round(toNumber(memoryTotalMb) * 1024 * 1024),
          source: 'nvidia-smi',
        });
      }
    );
...
async function getOverlayMetrics(config = {}) {
  ...
  await Promise.all([
    ...
    fpsService.getFpsMetrics({ enabled: !config.overlay || config.overlay.showFps !== false }).then((value) => {
      fpsMetrics = value;
    }).catch((err) => {
  ...
  return {
    ...
    cpu: {
      ...
      powerWatts: metrics ? round(metrics.temperatureSummary && metrics.temperatureSummary.cpuPowerWatts, 0) : null,
      powerSource: metrics ? (metrics.temperatureSummary && metrics.temperatureSummary.cpuPowerSource) || '' : '',
      temperatureC: metrics ? round(coreTempCpuTemperature(metrics), 0) : null,
      temperatureSource: metrics ? (metrics.temperatureSummary && metrics.temperatureSummary.cpuTemperatureSource) || '' : '',
      clockGhz: getCpuClockGhz(),
      cores: metrics ? metrics.cpu && metrics.cpu.cores : os.cpus().length,
    },
    gpu: {
      usagePercent: gpu ? gpu.usagePercent : null,
      temperatureC: gpu && gpu.temperatureC != null ? gpu.temperatureC : (metrics ? round(hottestGpuFromTemperatures(metrics), 0) : null),
      vramUsedBytes: gpu ? gpu.vramUsedBytes : null,
      vramTotalBytes: gpu ? gpu.vramTotalBytes : null,
      source: gpu ? gpu.source : ((metrics && metrics.temperatureSummary && metrics.temperatureSummary.gpuAvailable) ? 'temperature-sensor' : 'unavailable'),
    },
    ram: {
