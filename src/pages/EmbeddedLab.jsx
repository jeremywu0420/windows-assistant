...
const consoleStyle = {
  fontFamily: '"Cascadia Code","Consolas",monospace', fontSize: 12.5, lineHeight: 1.5,
  background: '#0d1117', color: '#d6deeb', borderRadius: 10, padding: 12,
  height: 280, overflowY: 'auto', whiteSpace: 'pre-wrap', wordBreak: 'break-word',
...
const colorFor = (stream) => (stream === 'stderr' || stream === 'error' ? '#ff7b72'
  : stream === 'system' ? '#7ee787' : '#d6deeb');
  ...
  return (
    <div style={consoleStyle} ref={ref}>
      {lines.length === 0
        ...
        : lines.map((l, i) => <span key={i} style={{ color: colorFor(l.stream) }}>{l.text}</span>)}
    </div>
  );
}
...
export default function EmbeddedLab() {
  ...
  const append = (setter) => (chunk) => setter((prev) => {
  ...
  useEffect(() => {
    ...
    return () => { offBuild && offBuild(); offSerial && offSerial(); };
  }, []);

  // Best-effort: refresh port list on mount.
  useEffect(() => { refreshPorts(); /* eslint-disable-next-line */ }, []);
  ...
  const connect = async () => {
    if (!port) { toast('請先選擇連接埠', 'error'); return; }
    setSerialLines([]);
  ...
  const disconnect = async () => { await window.api.closeSerial(); setConnected(false); };
    ...
    <div>
      ...
      >
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 10 }}>
          <code style={{ fontFamily: '"Cascadia Code","Consolas",monospace', fontSize: 12, wordBreak: 'break-all' }}>
            {folder || '尚未選擇資料夾'}
          </code>
          {detected ? (
            detected.supported
      ...
      <SectionPanel
        ...
        actions={(
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            <select style={selectStyle} value={port} onChange={(e) => setPort(e.target.value)} disabled={connected}>
              {ports.length === 0 ? <option value="">無連接埠</option>
                : ports.map((p) => <option key={p.path} value={p.path}>{p.label}</option>)}
            </select>
            <select style={selectStyle} value={baud} onChange={(e) => setBaud(Number(e.target.value))} disabled={connected}>
              {BAUDS.map((b) => <option key={b} value={b}>{b}</option>)}
            </select>
            <Button onClick={refreshPorts} disabled={connected}>重新整理</Button>
            {connected
              ? <Button variant="danger" onClick={disconnect}>關閉</Button>
              : <Button variant="primary" onClick={connect} disabled={!port}>連線</Button>}
            <Button variant="ghost" onClick={() => setSerialLines([])}>清除</Button>
          </div>
        )}
      >
        <div style={{ marginBottom: 8 }}>
          <StatusBadge tone={connected ? 'ok' : 'muted'}>{connected ? `已連線 ${port} @ ${baud}` : '未連線'}</StatusBadge>
        </div>
