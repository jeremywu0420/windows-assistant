...
export default function Topbar({ title, onOpenPalette, onNavigate }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      const configuredName = settings?.settings?.general?.displayName || settings?.settings?.general?.userName;
      if (configuredName) {
        const initials = configuredName.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase();
  ...
  return (
    <header className="topbar">
      ...
      <button type="button" className="tb-search" onClick={onOpenPalette} title={`${t('shell.search')} (Ctrl+K)`}>
        <LineIcon>
      ...
      <div className="tb-actions">
        <button type="button" className="tb-icon-btn" onClick={() => onNavigate && onNavigate('notifications')} title={t('shell.notifications')}>
          <LineIcon>
        ...
        <button type="button" className="tb-icon-btn" onClick={() => onNavigate && onNavigate('settings')} title={t('shell.settings')}>
          <LineIcon>
            <circle cx="12" cy="12" r="3.5" />
            <path d="M12 3v2M12 19v2M4.2 7.5l1.7 1M18.1 15.5l1.7 1M4.2 16.5l1.7-1M18.1 8.5l1.7-1M3 12h2M19 12h2" />
          </LineIcon>
        </button>
        <button type="button" className="tb-icon-btn" onClick={cycleTheme} title={t('shell.toggleTheme')}>
          <LineIcon>
        ...
        <button type="button" className="tb-user" onClick={() => onNavigate && onNavigate('settings')}>
